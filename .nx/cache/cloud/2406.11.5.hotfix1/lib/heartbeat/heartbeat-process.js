...
function lightClientRequire(moduleName) {
  ...
  let path;
  try {
    path = require.resolve(moduleName, { paths });
  } catch (e) {
  ...
  try {
    ...
    if (process.env.NX_VERBOSE_LOGGING === "true") {
      console.error(
        `Was not able require module ${moduleName} from path ${path}. This may be expected. `
      );
    }
    throw e;
  }
...
var require_nx_imports_light = __commonJS({
  "src/lib/utilities/nx-imports-light.ts"(exports) {
    ...
    try {
      try {
        const { output: output4 } = lightClientRequire("nx/src/utils/output");
        let workspaceRoot3;
        try {
          workspaceRoot3 = lightClientRequire("nx/src/utils/app-root").workspaceRoot;
        } catch (_a) {
          workspaceRoot3 = lightClientRequire(
            "nx/src/utils/workspace-root"
          ).workspaceRoot;
        }
        exports.workspaceRoot = workspaceRoot3;
        exports.output = output4;
      } catch (e) {
        const { output: output4 } = lightClientRequire(
...
__export(environment_exports, {
  ...
  NX_CLOUD_FORCE_METRICS: () => NX_CLOUD_FORCE_METRICS,
...
var init_environment = __esm({
  "src/lib/utilities/environment.ts"() {
    ...
    NX_CLOUD_FORCE_METRICS = process.env.NX_CLOUD_FORCE_METRICS === "true";
...
var get_cloud_options_exports = {};
__export(get_cloud_options_exports, {
  getCloudOptions: () => getCloudOptions
});
function getCloudOptions(taskRunnerConfiguration) {
  var _a, _b;
  const nxJson = JSON.parse(
    stripJsonComments((0, import_fs2.readFileSync)(`${workspaceRoot2}/nx.json`).toString(), {
      trailingCommas: true
    })
  );
  const result = {};
  ...
  return {
    ...
    nxCloudOptions: {
      ...result,
      ...(_b = (_a = nxJson.tasksRunnerOptions) == null ? void 0 : _a[taskRunnerConfiguration]) == null ? void 0 : _b.options
    }
  };
}
var import_fs2, workspaceRoot2;
...
var require_iterate = __commonJS({
  "node_modules/asynckit/lib/iterate.js"(exports, module2) {
    ...
    function iterate(list, iterator, state, callback) {
      ...
      state.jobs[key] = runJob(iterator, key, list[key], function(error, output4) {
...
function createApiAxiosInstance(options, timeout = 1e4) {
  ...
  if (options.customProxyConfigPath) {
    const { nxCloudProxyConfig } = require((0, import_path3.join)(process.cwd(), options.customProxyConfigPath));
...
var import_path3, output2, axios;
...
function getLatestCommitMessage() {
  try {
    const output4 = (0, import_child_process2.execSync)("git log -1 --pretty=%B", { encoding: "utf-8" });
    return output4.trim();
  } catch {
    return null;
  }
...
function getLatestCommitAuthor() {
  try {
    const output4 = (0, import_child_process2.execSync)("git log -1 --pretty=%aN", { encoding: "utf-8" });
    return output4.trim();
  } catch {
    return null;
  }
...
function getBranch2() {
  try {
    const output4 = (0, import_child_process2.execSync)("git branch --show-current", { encoding: "utf-8" });
    return output4.trim();
  } catch {
    return null;
  }
...
function getRef() {
  try {
    const output4 = (0, import_child_process2.execSync)("git rev-parse --abbrev-ref HEAD", {
      encoding: "utf-8"
    });
    return output4.trim();
  } catch {
...
var run_group_api_exports = {};
__export(run_group_api_exports, {
  RunGroupApi: () => RunGroupApi
});
var import_util, import_zlib, output3, RunGroupApi;
    ...
    RunGroupApi = class {
      ...
      async createRunGroup(branch, runGroup, ciExecutionId, ciExecutionEnv, stopAgentsOnFailure, agentCount, stopAgentsAfter, distributeOn, commitSha, commitRef, affectedProjectRatio, envVars, requireExplicitCompletion = false, touchedProjects = []) {
        ...
        try {
          ...
          output3.error({
            title: e.message,
            bodyLines: ((_b = (_a = e.response) == null ? void 0 : _a.data) == null ? void 0 : _b.split("\n")) || []
          });
          process.exit(1);
        }
      }
...
// src/lib/heartbeat/heartbeat-process.ts
var heartbeat_process_exports = {};
__export(heartbeat_process_exports, {
  createFileLogger: () => createFileLogger
});
module.exports = __toCommonJS(heartbeat_process_exports);
...
function getHeartbeatLockFilePath(runGroup) {
  const markerPath = (0, import_path2.join)((0, import_os.tmpdir)(), `run-group-${runGroup}-marker.lock`);
  return markerPath;
}

// src/lib/heartbeat/heartbeat-process.ts
function createFileLogger(runGroup) {
  const lockFilePath = getHeartbeatLockFilePath(runGroup);
  const logFilePath = `${lockFilePath}/logs.txt`;
  return {
    log: (message) => {
      const currentTime = (/* @__PURE__ */ new Date()).toISOString();
      (0, import_fs3.appendFileSync)(logFilePath, `[${currentTime}] - ${message}
`);
    }
  };
}
async function startHeartbeat() {
  var _a;
  const lightRunnerResolutionPaths = JSON.parse(
  ...
  configureLightClientRequire2(lightRunnerResolutionPaths);
  const { getCloudOptions: getCloudOptions2 } = (init_get_cloud_options(), __toCommonJS(get_cloud_options_exports));
  const { RunGroupApi: RunGroupApi2 } = (init_run_group_api(), __toCommonJS(run_group_api_exports));
  const { getRunGroup: getRunGroup2, getCIExecutionId: getCIExecutionId2 } = (init_environment(), __toCommonJS(environment_exports));
  const ciExecutionId = getCIExecutionId2();
  if (ciExecutionId == null)
    process.exit(0);
  const runGroup = getRunGroup2();
  const fileLogger = createFileLogger(runGroup);
  fileLogger.log(
    `Configured light client resolution for process ${process.pid}`
  );
  const { nxCloudOptions: options } = getCloudOptions2();
  const api = new RunGroupApi2(options);
  fileLogger.log("Created run group api");
  addProcessSignalListeners(fileLogger, runGroup);
  fileLogger.log("Added signal handlers for exit");
  (_a = process.stdout) == null ? void 0 : _a.write("heartbeat-ready");
  fileLogger.log("Starting to poll");
  await startPolling(fileLogger, api, ciExecutionId, runGroup);
  fileLogger.log("Polling failed, exiting");
}
function addProcessSignalListeners(fileLogger, runGroup) {
  const onSignal = (signal) => {
    const markerPath = getHeartbeatLockFilePath(runGroup);
    fileLogger.log(`Received ${signal}, Shutting down heartbeat process`);
    process.exit(0);
  };
  const signals = [
    "SIGINT",
...
async function sendHeartbeatWithRetries(fileLogger, api, ciExecutionId, runGroup) {
  fileLogger.log("Starting heartbeat request attempts");
  for (let attempt = 0; attempt < 5; ++attempt) {
    try {
      await api.sendHeartbeat(ciExecutionId, runGroup);
      fileLogger.log(`Request was successful after ${attempt + 1} attempt(s)`);
      return true;
    } catch (e) {
      fileLogger.log(`Request failed: ${JSON.stringify(e)}`);
    } finally {
      await wait(5e3);
    }
  }
  fileLogger.log("Failed to successfully contact api");
  return false;
}
async function startPolling(fileLogger, api, ciExecutionId, runGroup) {
  while (true) {
    const requestSuccessful = await sendHeartbeatWithRetries(
      fileLogger,
      api,
      ciExecutionId,
      runGroup
